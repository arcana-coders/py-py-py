...
def extract_asins():
    ...
    with open("c://amznfilescapalsa/htmlmx.html", "r", encoding="utf-8") as f:
        content = f.read()
    ...
    for product_link in soup.find_all('a'):
        ...
        if href and '/dp/' in href:
            ...
            if asin:  # añade ASIN solo si no es vacío
                asins.append(asin)
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                if 1 <= precio_num <= 499:
                    precio_venta = precio_num + 100
                elif 500 <= precio_num <= 1000:
                    precio_venta = precio_num + 175
                elif 1001 <= precio_num <= 1999:
                    precio_venta = precio_num + 250
                elif 2000 <= precio_num <= 3999:
                    precio_venta = precio_num + 350          
                elif precio_num >= 4000:
                    precio_venta = precio_num + 580
                else:
...
while os.path.exists(filename):
    filename = f"c:/amznfilescapalsa/product_mx_{datetime.now().strftime('%Y-%m-%d')}_{counter}.csv"
    counter += 1

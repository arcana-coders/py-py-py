...
def get_asins_from_page(url):
    ...
    # Busca en todos los enlaces de la página
    for link in soup.find_all('a'):
        print(link)
        href = link.get('href')
        # Asegúrate de que el enlace no sea None y que contenga '/dp/'
        if href and '/dp/' in href:
            # Extrae el ASIN de la URL
            asin = href.split('/dp/')[1].split('/')[0].split('?')[0]
            # Añade el ASIN a la lista de asins
            asins.append(asin)

    # Si no se encontraron ASINs en los enlaces, busca en el atributo 'data-asin' de las etiquetas <div>
    for div in soup.find_all('div', {'data-asin': True}):
        asin = div.get('data-asin')
        # Añade el ASIN a la lista de asins
        asins.append(asin)

    # Elimina duplicados convirtiendo la lista a un conjunto y luego de nuevo a una lista
            ...
            if response.status_code == 200:
                ...
                if div_imagenes is not None:
                    ...
                    for imagen_element in imagen_elements:
                        if len(imagenes) == 3:
                            break
                        imagen_url = imagen_element.get("src").replace('US40', 'SX679').replace('SX38_SY50_CR,0,0,38,50_' , 'SX679')

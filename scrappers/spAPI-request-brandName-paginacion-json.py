...
# Marca para la búsqueda
brand_name = "Weiman"  # Reemplaza con la marca que desees buscar
...
def make_requests(access_token, brand_name, next_token=None):
    endpoint = "https://sellingpartnerapi-na.amazon.com/catalog/2022-04-01/items"
    marketplace_id = "A1AM78C64UM0Y8"

    # Usamos 'keywords' en lugar de 'brandName' si la búsqueda por marca no es un parámetro disponible
    query_params = {
        "keywords": brand_name,
...
page_count = 1
while True:
    response_data, next_token = make_requests(access_token, brand_name, next_token)
    current_date = datetime.now().strftime("%Y-%m-%d")
    filename = f"{output_directory}\\{brand_name}_{current_date}.json"
    
    # Guardar los datos en el archivo JSON
    save_to_json(response_data, filename)
    print(f"Datos de la marca '{brand_name}' de la página {page_count} guardados en {filename}")
